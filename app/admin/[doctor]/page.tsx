...
export default async function page({ params }: { params: { doctor: string } }) {


    const doctor=decodeURIComponent(params.doctor);
    console.log("doctor",doctor);
    const appointment = await findDoctor(doctor);
    const count = await doctorAppointmentCount(doctor);
        ...
        <div>
            <nav className="text-white flex justify-between items-center p-3 h-[70px] bg-dark-200  rounded-3xl ">
                ...
                <div className="flex flex-row items-center justify-center gap-3">
                    {
                        <>
                            {/* <Image src={doctor[0].image} width={40} height={30} alt="doctor" /> */}
                            <div>{doctor}</div>
                        </>
                    }
                </div>
